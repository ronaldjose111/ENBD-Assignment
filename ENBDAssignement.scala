...
object ENBDAssignment extends App {
	...
	.config("spark.sql.shuffle.partitions", 3)//Setting number of partitions after Shuffle/Join to 3 for better performance in this case of small dataset, default is 200
	.getOrCreate()
	...
	val accountDfNew = valueDF1.select("value.*").withWatermark("AccountOpeningTime", "1 minute")//Spark doesnt support two streaming dataset Joins in complete/update mode, for using append mode we have to watermark the dataset.
	...
	val loanDfNew = valueDF2.select("value.*").withWatermark("LoanOpeningTime", "1 minute")//Spark doesnt support two streaming dataset Joins in complete/update mode, for using append mode we have to watermark the dataset.


	//Joining both Streams
	val joinExpr = expr("AccountId == AccountId_Lt AND LoanOpeningTime BETWEEN AccountOpeningTime AND AccountOpeningTime + interval 30 second")//Setting the maximum latency between the loan and account records as 30 seconds as per the problem statement
	val joinType = "inner"
	...
	//Window Aggregations
	val windowAggDf = joinedDF.groupBy(window(col("AccountOpeningTime"), "1 minute"), col("AccountType"))//Using tumbling window of 1 minute to calculate output in every minute as per the problem statement
	.agg(count("LoanId").alias("TotalCount"), (sum("Amount").alias("TotalAmount")))//Calculated last minute total count and amount. Complete output mode is not supported by spark in two streaming dataset joins, hence not calculating total amount and total count from beginning
	...
	.format("console")
	.outputMode("append")//Spark doesnt support two streaming dataset Joins in complete/update mode, hence using append mode
